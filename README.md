...
5. Place and fully fulfill a development order with an email and a real product. Set delay to zero **before fulfillment** for quick testing. Run `SHOPIFY_APP_URL=https://YOUR-CURRENT-TUNNEL npm run invitations:send` (use the HTTPS URL printed by the CLI), open the matching JSON in `work/outbox`, and follow its review URL. The default development sender writes files only; configure Resend under Email and operations below for actual delivery.
...
Invitations use the email captured from the fulfilled order. Set `EMAIL_PROVIDER=resend`, `RESEND_API_KEY`, and `EMAIL_FROM` (a verified Resend sender) in your local `.env` or deployment secrets. Set `SHOPIFY_APP_URL` to the publicly reachable HTTPS app/tunnel URL; localhost links are rejected for real email. Run `npm run invitations:send` to send due invitations, or keep `npm run invitations:watch` running for automatic checks every minute. The configured fulfillment delay still applies. Previously consumed or sent invitations are not resent.

The default `EMAIL_PROVIDER=development` remains file-only and is forbidden in production. Provider errors leave the invitation unsent. A successful API response means provider acceptance, not confirmed inbox delivery. Resend integration follows its [send-email API](https://resend.com/docs/api-reference/emails/send-email). Configure bounce/suppression handling and appropriate review-email consent handling before a public launch.

Run `npm run invitations:send` from **one scheduler/worker** every minute in development. Each run handles up to 100 due invitations. A failed delivery remains retryable. Token and delivery IDs are stable across retries; Resend receives a stable invitation ID as its idempotency key. Its [deduplication window is 24 hours](https://resend.com/docs/dashboard/emails/idempotency-keys); an ambiguous send retried after that window can duplicate an email and should be reconciled in Resend before restarting a long-stopped worker. Keep the sender, product title, and public URL stable during retries. No hosted scheduler has been deployed.
